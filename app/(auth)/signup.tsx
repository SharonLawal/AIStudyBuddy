...
import { Mail, Lock, User, GraduationCap, ArrowRight, Check, X } from 'lucide-react-native';
...
export default function SignUpScreen() {
  ...
  async function signUpWithEmail() {
    ...
    if (!isPasswordValid) {
      showNotification('error', 'Weak Password', 'Please ensure your password meets all requirements.');
      return;
    }
      ...
      >
        <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center' }} className="p-8">
          ...
          <View className="gap-5">
            <View>
              <Text className="text-foreground font-medium mb-2 ml-1">Full Name</Text>
              <Input 
            ...
            <View>
              <Text className="text-foreground font-medium mb-2 ml-1">Password</Text>
              <Input 
              ...
              {password.length > 0 && (
                <View className="mt-3 flex-row flex-wrap gap-2">
                  <ValidationItem label="8+ chars" isValid={validations.minLength} />
                  <ValidationItem label="Number" isValid={validations.hasNumber} />
                  <ValidationItem label="Uppercase" isValid={validations.hasUpper} />
                  <ValidationItem label="Special char" isValid={validations.hasSpecial} />
                </View>
              )}
            </View>
            ...
            <Button 
              ...
              size="lg" 
              className={`mt-4 rounded-2xl shadow-lg shadow-secondary/20 ${!isPasswordValid && password.length > 0 ? "opacity-50" : ""}`}
            >
...
function ValidationItem({ label, isValid }: { label: string, isValid: boolean }) {
  return (
    <View className={`flex-row items-center px-2 py-1 rounded-md border ${isValid ? "bg-green-500/10 border-green-500/20" : "bg-muted border-transparent"}`}>
      {isValid ? <Check size={12} className="text-green-600 mr-1" /> : <X size={12} className="text-muted-foreground mr-1" />}
      <Text className={`text-xs ${isValid ? "text-green-700 font-medium" : "text-muted-foreground"}`}>{label}</Text>
    </View>
  );
}